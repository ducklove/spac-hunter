...
공모가는 `overrides.json` > 증권신고서 추출값(OpenDART 키 설정 시) > 기본 2,000 순서로 결정됩니다. 청산일은 `overrides.json`에 값이 없으면 KIND 상장일 + 36개월로 추정합니다. 일반 운영비, 상장비, 합병 추진비는 기본 계산에서 공모예치금 차감 항목으로 보지 않습니다. 실제 예치기관 수익률, 세금, 확정 분배금은 공시 확인 후 `overrides.json`으로 보정합니다. 별도 공시나 수동 입력이 없으면 예상 예치이자는 KOFR 최신 공시금리를 fallback 금리로 사용합니다.
...
- 종목별 스파크라인
- 평균 기대수익률 추이: 일별 종가·해당 날짜의 청산 잔여일수로 연환산한 수익률의 동일 비중 평균, 1M/3M/6M/전체 기간 선택과 날짜별 수익률·표본 수 툴팁. 현재 상장 종목과 최신 청산분배금 추정치로 재계산하며, 계산 가능한 종목의 70% 이상에 유효 이력이 있는 날짜만 표시합니다. 과거 시점의 추정치나 상폐 종목을 복원한 통계는 아니며, 실시간 시세와 별도로 일일 수집 시 갱신됩니다.
- CSV 내보내기
