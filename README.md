...
차감 가정은 실행 옵션으로 바꿀 수 있습니다: `--trust-fee-pct`(기본 0.1), `--interest-tax-pct`(기본 15.4), `--payout-lag-days`(상장폐지→분배, 기본 102).
...
예치금 계산 보정 키: `trustFeePct`(신탁보수 %p, 예금형이면 0), `interestTaxPct`(이자 원천징수 %), `payoutDate`(잔여재산 분배일이 공시되면 입력, 연환산 기준일).
