...
from .sources import dart, opendart
...
_AGENT_CONTEXT_LABEL_RE = re.compile(
    r"예치\s*기관|예치처|예치\s*은행|신탁\s*업자|수탁\s*기관|보관\s*기관|"
    r"공모\s*자금|예치\s*금|예치\s*\(\s*신탁\s*\)"
)
_AGENT_PATTERNS = (
    ("한국증권금융", re.compile(r"한국\s*증권\s*금융(?:\s*주식회사)?")),
    ("국민은행", re.compile(r"(?:KB\s*)?(?:국민\s*은행|\(주\)\s*국민\s*은행|주식회사\s*국민\s*은행)")),
    ("신한은행", re.compile(r"(?:\(주\)\s*)?(?:신한\s*은행|주식회사\s*신한\s*은행)")),
    ("우리은행", re.compile(r"(?:\(주\)\s*)?(?:우리\s*은행|주식회사\s*우리\s*은행)")),
    ("하나은행", re.compile(r"(?:KEB\s*)?(?:하나\s*은행|\(주\)\s*하나\s*은행|주식회사\s*하나\s*은행)")),
    ("농협은행", re.compile(r"(?:NH\s*)?(?:농협\s*은행|\(주\)\s*농협\s*은행|주식회사\s*농협\s*은행)")),
    ("기업은행", re.compile(r"(?:IBK\s*)?(?:기업\s*은행|중소\s*기업\s*은행)")),
    ("부산은행", re.compile(r"(?:BNK\s*)?부산\s*은행")),
    ("대구은행", re.compile(r"(?:iM\s*)?대구\s*은행")),
    ("광주은행", re.compile(r"광주\s*은행")),
    ("경남은행", re.compile(r"(?:BNK\s*)?경남\s*은행")),
    ("전북은행", re.compile(r"전북\s*은행")),
    ("수협은행", re.compile(r"(?:Sh\s*)?수협\s*은행")),
    ("산업은행", re.compile(r"(?:KDB\s*)?산업\s*은행|한국\s*산업\s*은행")),
    ("SC제일은행", re.compile(r"(?:SC\s*)?제일\s*은행")),
    ("한국씨티은행", re.compile(r"(?:한국\s*)?씨티\s*은행|Citibank", re.IGNORECASE)),
)
...
def _agent_search_windows(text):
    normalized = re.sub(r"\s+", " ", str(text or "")).strip()
    if not normalized:
        return []
    windows = []
    for match in _AGENT_CONTEXT_LABEL_RE.finditer(normalized):
        start = max(0, match.start() - 80)
        end = min(len(normalized), match.end() + 180)
        windows.append(normalized[start:end])
    if not windows and len(normalized) <= 120:
        windows.append(normalized)
    return windows


def clean_escrow_agent(value):
    """Return a normalized escrow institution name or ``None`` for generic fragments."""

    text = re.sub(r"\s+", " ", str(value or "")).strip(" -·:|,.")
    if not text:
        return None
    text = text.replace("㈜", "(주)")
    for window in _agent_search_windows(text):
        for label, pattern in _AGENT_PATTERNS:
...
def _extract_escrow_agent(text, warnings):
    agent = clean_escrow_agent(text)
    if agent:
        return agent
    if _AGENT_CONTEXT_LABEL_RE.search(str(text or "")):
...
def _extract_trust_change_start_date(text, fallback_date, warnings):
    for label in _TRUST_CHANGE_DATE_LABELS:
        for match in re.finditer(re.escape(label), text):
            ...
            if parsed:
                return parsed
    dates = [_to_iso_date(*match.groups()) for match in _FULL_DATE_RE.finditer(text)]
    dates = [parsed for parsed in dates if parsed]
    non_filing_dates = [parsed for parsed in dates if parsed != fallback_date]
    if non_filing_dates:
        return non_filing_dates[-1]
    if fallback_date:
...
def load_filings(path=None):
    ...
    for entry in filings.values():
        if isinstance(entry, dict) and "escrowAgent" in entry:
            entry["escrowAgent"] = clean_escrow_agent(entry.get("escrowAgent"))
    return {
        "filings": filings,
...
def _document_trust_rate_change_fields(receipt_no, filing_date=None, fetcher=None, source_label="OpenDART"):
    """Fetch + extract one trust-contract change document."""
    fetcher = fetcher or opendart.fetch_document_text
    try:
        text = fetcher(receipt_no)
    except Exception as exc:  # noqa: BLE001
        logger.warning("%s 신탁계약 변경 문서 추출 실패 %s: %s", source_label, receipt_no, exc)
...
def _filing_entry(receipt_no, report_name, filing_date, url, fields, now):
    ...
    for key in FIELD_KEYS:
        ...
        entry[key] = clean_escrow_agent(value) if key == "escrowAgent" else value
...
def _is_trust_rate_change_row(row):
    title = str(row.get("report_nm") or row.get("title") or row.get("reportName") or "")
    compact = re.sub(r"[\sㆍ·・.\[\]()]", "", title)
    return "신탁계약내용변경" in compact or ("신탁계약" in compact and "변경" in compact)
...
def _row_receipt_no(row):
    return str(_row_value(row, "rcept_no", "receiptNo", "receipt_no") or "").strip()


def _trust_rate_change_entry(row, fields):
    receipt_no = _row_receipt_no(row)
    filing_date = _format_rcept_dt(_row_value(row, "rcept_dt", "date", "filingDate"))
    report_name = re.sub(
        r"\s+", " ", str(_row_value(row, "report_nm", "title", "reportName") or "")
    ).strip() or None
...
def _trust_rate_change_needs_refresh(change):
    if not isinstance(change, dict):
        return False
    if parse_float(change.get("ratePct")) is None or not change.get("startDate"):
        return True
    try:
        version = int(change.get("parserVersion") or 0)
    except (TypeError, ValueError):
        version = 0
    return version < TRUST_RATE_CHANGE_PARSER_VERSION
...
def backfill_trust_rate_changes(store, items, listing_dates, doc_budget, today=None, now=None):
    ...
    The search combines OpenDART list.json and DART public detail search.
    Document text is downloaded only for unseen trust-contract change receipts
    and is bounded by ``doc_budget``.
    """
    errors = {}
    used = 0
    if doc_budget <= 0:
    ...
    try:
        ...
        errors["corpCode"] = str(exc)
        corp_map = {}
        logger.warning("OpenDART corpCode 매핑 실패, DART 공시통합검색으로 신탁계약 변경 백필 계속: %s", exc)

    opendart_available = True
    public_dart_available = True
    for item in items:
        if used >= doc_budget:
            break
        code = item["code"]
        ...
        if public_dart_available:
            try:
                public_rows = dart.fetch_dart_trust_rate_change_disclosures(
                    code, item.get("name"), listing_date, today
                )
            except Exception as exc:  # noqa: BLE001
                public_dart_available = False
                errors.setdefault(code, f"DART 신탁계약 변경 검색 실패: {exc}")
                logger.warning("DART 신탁계약 변경 검색 실패 %s(%s): %s", item.get("name"), code, exc)
            else:
                trust_rows.extend(
                    {**row, "_documentSource": "dart"}
                    for row in public_rows
                    if _is_trust_rate_change_row(row)
                )

        if corp_code and opendart_available:
            try:
                rows = opendart.fetch_filing_list(bgn_de, end_de, corp_code=corp_code)
            except Exception as exc:  # noqa: BLE001
                opendart_available = False
                errors.setdefault(code, f"OpenDART 신탁계약 변경 목록 조회 실패: {exc}")
                logger.warning(
                    "OpenDART 신탁계약 변경 목록 조회 실패 %s(%s): %s", item.get("name"), code, exc
                )
            else:
                trust_rows.extend(
                    {**row, "_documentSource": "opendart"} for row in rows if _is_trust_rate_change_row(row)
                )

        deduped_rows = {}
        for row in trust_rows:
            receipt_no = _row_receipt_no(row)
            row_date = _format_rcept_dt(_row_value(row, "rcept_dt", "date"))
            row_title = _row_value(row, "report_nm", "title")
            key = receipt_no or f"{row_date}|{row_title}"
            if key and key not in deduped_rows:
                deduped_rows[key] = row
        trust_rows = sorted(
            deduped_rows.values(),
            key=lambda row: (
                str(_format_rcept_dt(_row_value(row, "rcept_dt", "date", "filingDate")) or ""),
                _row_receipt_no(row),
            ),
        )
        if not trust_rows:
        ...
        seen_receipts = {
            ...
            and not _trust_rate_change_needs_refresh(change)
        }
        additions = []
        for row in trust_rows:
            if used >= doc_budget:
            ...
            used += 1
            filing_date = _format_rcept_dt(_row_value(row, "rcept_dt", "date", "filingDate"))
            if row.get("_documentSource") == "dart":
                fields = _document_trust_rate_change_fields(
                    receipt_no,
                    filing_date=filing_date,
                    fetcher=dart.fetch_dart_document_text,
                    source_label="DART",
                )
            else:
